...
- Optional encryption at rest for report blobs and document numbers. **v0.5.0:** AES-256-GCM on optional `STORE_RAW_REPORTS` files (`data/reports/{id}.bin`) and on `IdentityDocument.number` when `REPORT_BLOB_KEY` is set. Stored `numberNorm` is a keyed HMAC so exact match and list search of a full normalized number still work. No key keeps leftover plaintext (residual risk). Wrong/missing key on ciphertext rows fails closed. Do not encrypt `e164` / `emailNorm`. See [Roadmap.md](../07-ROADMAP/Roadmap.md).
- Optional local operator login. **v0.5.0:** `AUTH_ENABLED=true` plus `AUTH_SESSION_SECRET` (64 hex) and `AUTH_OPERATORS` (`user:password[,…]`) gates the web UI and tRPC behind an HttpOnly session cookie. Off by default (unauthenticated local-dev). Enabled without secret or operators fails closed. Not multi-tenant SaaS; no public signup. Residual risk: default deploy has no login; env stores operator passwords; login does not encrypt disk, SQL dumps, or leftover plaintext numbers; no lockout.
