...
Research (see Roadmap) picks **optional at-rest encryption of report blobs** as the first v0.5 increment, then **document-number ciphertext + HMAC** (PRD Risk / Data-Handling / GDPR Art. 32). `STORE_RAW_REPORTS` writes optional `data/reports/{id}.bin`; with `REPORT_BLOB_KEY` the file is an AES-256-GCM envelope and IdentityDocument numbers are sealed (HMAC of `numberNorm` stays searchable). Multi-user auth, graph visualization, and `pg_trgm` stay later. Vehicle photo, bank-name matching, PAN-as-identity, and employment graph stay out.
